...
Custom Webanalyze technology files can be passed per manual run or campaign with `--webanalyze-apps`. For queued campaigns, put private files in `custom-assets/` and reference them through `/home/zero/custom-assets/...` so the worker container can read them.

## Validation

- `nuclei`: active validation for selected templates, CVE IDs, tags, or custom template paths.

For broad campaigns, prefer focused templates over broad generic scanning. `httpx` and Webanalyze have dedicated batch timeouts; `ZERO_TOOL_TIMEOUT` bounds the remaining external steps such as subfinder, dnsx, Nuclei, and template updates.

Custom Nuclei templates should also live in `custom-assets/` for local Docker Compose runs. See [Custom Campaigns](CUSTOM_CAMPAIGNS.md) for safe template structure and campaign recipes.
